...
This library simplify the access to the data of the workflow. It centralizes workflow services through workflowBeans.

## Configuration

Inject the workflowBeanService corresponding to the adequate ressource workflow

```

 <bean id="identityimport.batchWFBean.wfbeanservice" class="fr.paris.lutece.plugins.identityimport.wf.WorkflowBeanService">
         <constructor-arg index="0" value="IDENTITYIMPORT_BATCH_RESOURCE" />
         <constructor-arg index="1" type="int"> <value>1 </value> </constructor-arg>   <!-- : the worlkflow ID (key) -->
 </bean>
...
Template integration : the workflowBean is a resource wrapper thats provides the state and the available actions for the resource :

(Example with a "Batch" class)
...
```
 ...
 <@box>
     <@boxHeader title='Workflow' />
         ...
         <!-- affichage du statut -->
        statut :  <#if batchWFBean.state??> <@tag  color='warning'>${batchWFBean.state.name} </@tag> <#else> not set </#if>
        
         <!-- affichage des actions ->>
         <#if batchWFBean.actions??>actions :
         <#list batchWFBean.actions as action >
             <@aButton href="jsp/admin/plugins/identityimport/ManageBatchs.jsp?action=processAction&id_action=${action.id}&id=${batchWFBean.resource.id}"
         ...
         <!-- affichage historique ressource -->
        History : ${batchWFBean.history! }
...
JSPBean integration : the workflowBean can be created with _wfBeanService.createWorkflowBean()
